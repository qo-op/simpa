...
var SplitPane = /** @class */ (function () {
    ...
    SplitPane.pointerdown = function (ev) {
        ...
        if (currentTarget === document) {
            ...
            splitPane = document.evaluate("ancestor-or-self::*[contains(concat(' ', @class, ' '), ' SplitPane ')]", target, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        ...
        var divider = splitPane.children[1];
        if (divider !== target && !divider.contains(target)) {
            return;
        }
        try {
            var leftComponent_1 = splitPane.children[0];
            var rightComponent_1 = splitPane.children[2];
            var leftComponentRect = leftComponent_1.getBoundingClientRect();
            var rightComponentRect = rightComponent_1.getBoundingClientRect();
            var leftComponentComputedStyle = getComputedStyle(leftComponent_1);
            var rightComponentComputedStyle = getComputedStyle(rightComponent_1);
            var verticalSplit_1 = splitPane.dataset.orientation === "vertical-split";
            var pageEndSplitPane_1 = splitPane.dataset.dividerAnchor === "page-end";
            var lineEndSplitPane_1 = splitPane.dataset.dividerAnchor === "line-end";
            var offset_1 = 0;
            var maximumDividerLocation_1 = 0;
            if (verticalSplit_1) {
                if (pageEndSplitPane_1) {
                    offset_1 = ev.clientY + rightComponentRect.height;
                }
                else {
                    offset_1 = ev.clientY - leftComponentRect.height;
                }
                maximumDividerLocation_1 = leftComponentRect.height - +leftComponentComputedStyle.borderTopWidth.replace("px", "") - +leftComponentComputedStyle.borderBottomWidth.replace("px", "");
                maximumDividerLocation_1 += rightComponentRect.height - +rightComponentComputedStyle.borderTopWidth.replace("px", "") - +rightComponentComputedStyle.borderBottomWidth.replace("px", "");
            }
            else {
                if (lineEndSplitPane_1) {
                    offset_1 = ev.clientX + rightComponentRect.width;
                }
                else {
                    offset_1 = ev.clientX - leftComponentRect.width;
                }
                maximumDividerLocation_1 = leftComponentRect.width - +leftComponentComputedStyle.borderLeftWidth.replace("px", "") - +leftComponentComputedStyle.borderRightWidth.replace("px", "");
                maximumDividerLocation_1 += rightComponentRect.width - +rightComponentComputedStyle.borderLeftWidth.replace("px", "") - +rightComponentComputedStyle.borderRightWidth.replace("px", "");
            }
            var dragLayer_1 = document.createElement("div");
            dragLayer_1.classList.add("DragLayer");
            if (verticalSplit_1) {
                dragLayer_1.style.cursor = "ns-resize";
            }
            else {
                dragLayer_1.style.cursor = "ew-resize";
            }
            document.body.appendChild(dragLayer_1);
            var dragLayerEventListener = {
                pointermove: function (ev) {
                    if (verticalSplit_1) {
                        if (pageEndSplitPane_1) {
                            var location_1 = Math.min(Math.max(offset_1 - ev.clientY, 0), maximumDividerLocation_1);
                            rightComponent_1.style.height = location_1 + "px";
                        }
                        else {
                            var location_2 = Math.min(Math.max(ev.clientY - offset_1, 0), maximumDividerLocation_1);
                            leftComponent_1.style.height = location_2 + "px";
                        }
                    }
                    else {
                        if (lineEndSplitPane_1) {
                            var location_3 = Math.min(Math.max(offset_1 - ev.clientX, 0), maximumDividerLocation_1);
                            rightComponent_1.style.width = location_3 + "px";
                        }
                        else {
                            var location_4 = Math.min(Math.max(ev.clientX - offset_1, 0), maximumDividerLocation_1);
                            leftComponent_1.style.width = location_4 + "px";
                        }
                    }
                },
            ...
            dragLayer_1.addEventListener("pointermove", dragLayerEventListener.pointermove);
            dragLayer_1.addEventListener("pointerup", dragLayerEventListener.pointerup);
            dragLayer_1.addEventListener("pointerleave", dragLayerEventListener.pointerleave);
        }
        finally {
            ev.stopPropagation();
        }
...
var Tree = /** @class */ (function () {
    ...
    Tree.pad = function (ul, paddingInlineStart, padding) {
        ul.querySelectorAll("\n\t\t\t\t:scope>li\n\t\t").forEach(function (li) {
            if (li.children.length === 0) {
                return;
            }
            var firstChild = li.children[0];
            firstChild.style.paddingInlineStart = paddingInlineStart + "px";
            var _ul = li.querySelector(":scope>ul");
            if (_ul !== null) {
                Tree.pad(_ul, paddingInlineStart + padding, padding);
            }
        });
    };
    Tree.select = function (li, ctrlKey) {
        if (ctrlKey) {
    ...
    Tree.clearSelection = function (ul) {
        ul
    ...
    Tree.click = function (ev) {
        ...
        if (currentTarget === document) {
            ...
            tree = document.evaluate("ancestor-or-self::*[contains(concat(' ', @class, ' '), ' Tree ')]", target, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
            if (!tree) {
                return;
            }
            if (!tree.onclick) {
            ...
            tree = currentTarget;
            li = document.evaluate("ancestor-or-self::li[position() = 1]", target, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
        }
        if (li === null) {
            return;
        }
        try {
            var singleTreeSelection = tree.dataset.selectionMode === "single-tree-selection";
            if (singleTreeSelection) {
                ...
                if (ev.shiftKey) {
                    var leadSelection = tree.querySelector(":scope li[data-lead-selection]");
                    var lis = tree.querySelectorAll("\n\t\t\t\t\t\t\t:scope li\n\t\t\t\t\t");
                    var start = null;
            ...
            if (!ev.ctrlKey) {
                var treeNode = li.querySelector(":scope>.TreeNode");
                if (treeNode !== null) {
